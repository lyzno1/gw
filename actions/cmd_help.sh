...
cmd_help() { # Renamed from show_help
    ...
    printf "  ${YELLOW}%-22s${NC} %s\n" "update [--merge|-m]" "- 更新当前分支: 若为特性分支则与主干同步, 若为主干则拉取。"
    printf "  %-22s  ${GRAY}(自动处理 stash, 默认使用 rebase, --merge/-m 使用 merge)${NC}\n" ""
    ...
    # --- Worktree 管理命令 ---
    echo -e "${CYAN}🌲 Worktree 管理命令 (Git Worktree Management) 🌲${NC}"
    printf "  ${YELLOW}%-22s${NC} %s\n" "wt-init" "- 初始化worktree环境，设置目录结构和配置。"
    printf "  %-22s  ${GRAY}(创建main/和dev/目录，更新.gitignore)${NC}\n" ""
    printf "  ${YELLOW}%-22s${NC} %s\n" "wt-start <branch>" "- 创建新的worktree分支并开始工作。"
    printf "  %-22s  ${GRAY}(支持 --base <base>指定基础分支, --local/-l本地模式)${NC}\n" ""
    printf "  ${YELLOW}%-22s${NC} %s\n" "wt-list" "- 列出所有worktree及其状态信息。"
    printf "  %-22s  ${GRAY}(支持 --detailed/-d详细信息, --simple/-s简单模式)${NC}\n" ""
    printf "  ${YELLOW}%-22s${NC} %s\n" "wt-switch <branch>" "- 切换到指定的worktree目录。"
    printf "  ${YELLOW}%-22s${NC} %s\n" "wt-update" "- 在当前worktree中同步主分支变更。"
    printf "  %-22s  ${GRAY}(自动处理stash，更新主分支，rebase当前分支)${NC}\n" ""
    printf "  ${YELLOW}%-22s${NC} %s\n" "wt-submit" "- 提交当前worktree的工作并可选自动清理。"
    printf "  %-22s  ${GRAY}(支持 --auto-clean自动清理, --keep-worktree保留环境)${NC}\n" ""
    printf "  ${YELLOW}%-22s${NC} %s\n" "wt-clean <branch>" "- 清理指定的worktree和分支。"
    printf "  %-22s  ${GRAY}(支持 --force/-f强制清理, --keep-branch/-k保留分支)${NC}\n" ""
    printf "  ${YELLOW}%-22s${NC} %s\n" "wt-prune" "- 清理所有无效的worktree引用和目录。"
    printf "  %-22s  ${GRAY}(支持 --dry-run/-n预览, --force/-f强制清理)${NC}\n" ""
    echo ""
    echo -e "${CYAN}💡 Worktree典型工作流：${NC}"
    echo -e "  ${GRAY}1. 初始化:${NC}      gw wt-init"
    echo -e "  ${GRAY}2. 开始工作:${NC}    gw wt-start feature-login"
    echo -e "  ${GRAY}3. 进入目录:${NC}    cd dev/feature-login"
    echo -e "  ${GRAY}4. 日常开发:${NC}    gw save \"implement login\""
    echo -e "  ${GRAY}5. 同步主分支:${NC}  gw wt-update"
    echo -e "  ${GRAY}6. 提交工作:${NC}    gw wt-submit --auto-clean"
    echo ""
