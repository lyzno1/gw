...
show_help() {
    ...
    echo "  config <user> <email> [--global] - 快速设置本地 (默认) 或全局 (使用 --global) 的 user.name 和 user.email。"
    echo "  config [...]            - 执行原生 git config 命令 (例如: gw config --system core.autocrlf input)。"
    ...
    echo "  sp [-m \"消息\"] [-e] [文件...] - 快速保存所有当前变更并推送到远程 (等效于 gw save ... && gw push)。"
    echo "  sync                    - 同步开发分支: 拉取主分支 ('$MAIN_BRANCH') 最新代码并 rebase 当前分支。"
    echo "                            (如果当前有未提交的更改，会提示您暂存后再继续。)"
    echo "                            (在主分支上执行时，仅拉取最新代码。)"
    echo "                            (在开发分支上执行时，会先更新主分支，再 rebase 当前分支。)"
    ...
    echo "  stash [子命令] [参数]    - 工作区变更暂存与恢复，支持常用 git stash 子命令封装。"
    echo -e "    ${YELLOW}子命令:${NC}"
    echo -e "      ${GREEN}push${NC} [-m <msg>]     - 保存当前更改到 stash (默认)"
    echo -e "      ${GREEN}pop${NC} [<stash@{n}>]   - 弹出最近或指定 stash"
    echo -e "      ${GREEN}apply${NC} [<stash@{n}>] - 应用 stash（不删除）"
    echo -e "      ${GREEN}list${NC}                - 列出所有 stash"
    echo -e "      ${GREEN}show${NC} [<stash@{n}>]  - 显示 stash 详情"
    echo -e "      ${GREEN}drop${NC} <stash@{n}>    - 删除指定 stash"
    echo -e "      ${GREEN}clear${NC}               - 清空所有 stash（需确认）"
    echo ""
    ...
    echo "  pull [远程] [分支] [--no-rebase|--ff-only|...] - 拉取更新 (默认使用 --rebase 策略)。"
    echo "                            (支持原生 git pull 参数，可覆盖默认rebase行为)。"
    echo "  push [远程] [分支] [...] - 推送本地提交到远程 (带重试, 自动处理 -u, 支持原生 git push 参数)。"
    echo "  fetch [远程] [...]      - 从远程获取最新信息，但不合并 (支持原生 git fetch 参数)。"
    echo ""
    echo -e "${CYAN}其他分支操作:${NC}"
    echo "  branch                  - (无参数) 显示美化版的分支列表 (本地和远程跟踪)。"
    echo "  branch [...]            - (带参数) 执行原生 'git branch' 操作 (如: -a, -r, -d <名>)。"
    echo "                            (创建推荐: gw new <名>, 删除推荐: gw rm <名>)"
    echo "  checkout <分支名>       - 切换到已存在的分支 (会检查未提交变更)。"
    echo "  merge <来源分支> [...]  - 合并指定分支到当前分支 (支持原生 git merge 参数)。"
    echo "  rm <branch|all> [-f]      - 删除本地分支，并询问是否删除远程同名分支。"
    echo "                            gw rm <branch_name> [-f]"
    echo "                              删除指定的本地分支。删除成功后会检查并询问是否删除远程仓库 (${REMOTE_NAME}) 上的同名分支。"
    echo "                              -f: 强制删除本地分支 (使用 -D)。"
    echo "                            gw rm all [-f]"
    echo "                              清理所有其他本地分支 (需在主分支 '$MAIN_BRANCH' 运行)。"
    echo "                              过程分为两阶段:"
    echo "                                1. 自动识别并提示删除已合并到主分支的分支。"
    echo "                                2. 列出剩余的其他分支，提供选项:"
    echo "                                   - [M] 手动逐个审核 (可选安全/强制删除)。"
    echo "                                   - [F] 强制删除所有剩余分支 (危险! 需 Y/n 确认)。"
    echo "                                   - [S] 跳过所有剩余分支。"
    echo "                              (删除每个本地分支后都会检查并询问是否删除远程同名分支)"
    echo "                              -f 选项会影响所有本地删除操作。"
    echo ""
    ...
    echo ""
    echo -e "${CYAN}✨ Rebase 操作  ✨${NC}"
    echo "  rebase <目标分支> [其他参数] - 将当前分支 rebase 到指定的目标分支。"
    echo "                            (目标分支会自动从远程更新。)"
    echo "  rebase [-i|--interactive] <commit-ish> [其他参数] - 执行交互式 rebase 或其他原生 rebase 操作。"
    echo "  rebase --continue       - 继续一个正在进行的 rebase 操作。"
    echo "  rebase --abort          - 中止一个正在进行的 rebase 操作。"
    echo "  rebase --skip           - 跳过当前补丁并继续 rebase。"
    echo ""
