...
|-- actions/             -- 各个子命令的具体实现脚本
    |-- cmd_*.sh           # 实现具体命令逻辑 (如 cmd_new.sh, cmd_sync.sh, cmd_ide.sh)
...
*   **`cmd_ide.sh` (`gw ide`)**:
    *   **[新]** 用于设置 `gw save` 在编辑提交信息时默认使用的编辑器。
    *   用户可以通过预定义的短名称（如 `vscode`, `cursor`, `vim`，不区分大小写）或提供完整的编辑器命令字符串（需包含 `--wait` 等使编辑器阻塞的参数，如果适用）来进行配置。
    *   配置信息保存在用户主目录下的 `~/.gw_editor_pref` 文件中。
    *   当用户设置的命令可能无效时（如命令在 PATH 中找不到），会提供引导性警告。
*   **`cmd_sync.sh` (`gw sync`)**:
